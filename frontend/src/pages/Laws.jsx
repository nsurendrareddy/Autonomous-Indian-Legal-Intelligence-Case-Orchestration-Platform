...
const DEMO_SECTIONS = [
    // ── CONSUMER RIGHTS ──
    { title: "Consumer Protection Act 2019 — Deficiency in Service", section: "Section 2(11)", category: "Consumer Rights", description: "Defines deficiency as any fault, imperfection, shortcoming, or inadequacy in quality or manner of service.", applicability: "All consumers of goods/services", remedy: "Refund, compensation, or replacement", authority: "District Consumer Disputes Redressal Commission" },
    ...
    { title: "Prize Chits and Money Circulation Act 1978", section: "Section 4 & 5", category: "Financial Fraud", description: "Prohibits promotion of money circulation schemes promising multi-level returns (MLM fraud).", applicability: "MLM fraud victims", remedy: "Imprisonment up to 3 years + fine", authority: "State Police Economic Offences Wing" },

    // ── CIVIL LAW ──
    { title: "Limitation Act 1963 — Time Limit for Filing Cases", section: "Section 3", category: "Civil Law", description: "Sets the time limit within which a legal suit must be filed (typically 3 years for money suits, 12 years for property).", applicability: "Any person seeking to file a civil suit", remedy: "Case dismissed if filed after limitation period", authority: "Civil Court" },
    { title: "Specific Relief Act 1963", section: "Section 10", category: "Civil Law", description: "Allows courts to enforce performance of contracts involving immovable property or rare movables.", applicability: "Parties whose specific contract terms are breached", remedy: "Court order for specific performance of contract", authority: "Civil Court" },
    { title: "Code of Civil Procedure 1908 — Injunction", section: "Order 39 Rule 1 & 2", category: "Civil Law", description: "Courts can grant temporary injunctions to prevent irreparable harm during pending litigation.", applicability: "Any party needing urgent protection from irreparable harm", remedy: "Stay order / temporary injunction", authority: "Civil Court" },
    { title: "Indian Contract Act 1872 — Breach of Contract", section: "Section 73 & 74", category: "Civil Law", description: "Aggrieved party in a breach of contract may recover damages that naturally arise or were reasonably foreseeable.", applicability: "Any party to a breached written or verbal contract", remedy: "Compensation for loss or pre-agreed liquidated damages", authority: "Civil Court / Arbitration" },

    // ── WOMEN'S RIGHTS ──
    { title: "Protection of Children from Sexual Offences (POCSO) Act 2012", section: "Section 4 & 6", category: "Women's Rights", description: "Protects children below 18 from sexual assault, harassment, and exploitation with stringent punishments.", applicability: "Children under 18 who are victims of sexual offences", remedy: "Imprisonment of 7 years to life; child-friendly court procedures", authority: "Special POCSO Court / Child Welfare Committee" },
    ...
    // ── RTI & ENVIRONMENT ──
    { title: "Right to Information Act 2005", section: "Section 6", category: "RTI & Environment", description: "Every citizen can request information from any public authority within 30 days; PIO must respond.", applicability: "Any citizen seeking government information", remedy: "Penalty of ₹250/day up to ₹25,000 on PIO for delay", authority: "Public Information Officer / CIC / SIC" },
    { title: "Environment Protection Act 1986", section: "Section 15", category: "RTI & Environment", description: "Punishes industries violating pollution norms with imprisonment up to 5 years and fine up to ₹1 lakh.", applicability: "Citizens affected by industrial pollution, illegal construction", remedy: "Closure of industry, compensation, cleanup order", authority: "CPCB / SPCB / NGT (National Green Tribunal)" },
    { title: "National Green Tribunal Act 2010", section: "Section 14 & 15", category: "RTI & Environment", description: "NGT handles environmental disputes and can award compensation for environmental harm or illness.", applicability: "Persons affected by environmental damage, river pollution", remedy: "Compensation + restoration orders; no court fee required", authority: "National Green Tribunal (NGT)" },
];
    ...
    const load = async () => {
        ...
        try {
            const [secRes, catRes] = await Promise.all([
                axios.get(`${API}/api/legal-sections`, { timeout: 10000 }),
                axios.get(`${API}/api/legal-sections/categories`, { timeout: 10000 })
            ]);
            const fetched = secRes.data.sections || [];
            if (fetched.length > 0) {
                setSections(fetched);
                ...
                const cats = [...new Set(DEMO_SECTIONS.map(s => s.category))];
        ...
        <div className="page-container">
            <div className="page-header">
                <motion.h1 className="gradient-text" initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
                    📚 Indian Legal Sections
                </motion.h1>
                <p>Browse and search applicable laws for your legal situation</p>
            </div>

            {isDemo && (
                <motion.div className="demo-banner" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
                    <FaWifi className="demo-icon" />
                    <span><strong>Demo Mode</strong> — Backend not connected. Showing sample data.</span>
            ...
            {/* Search */}
            <motion.div className="laws-search glass" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
            ...
            {loading ? (
                ...
                <div className="no-results glass">
                    <p>No legal sections found matching your search.</p>
                </div>
            ) : (
                <div className="laws-grid">
                    {filtered.map((s, i) => (
                        <motion.div
                            key={i}
                            className="law-card glass glass-hover"
                        ...
                        >
                            <div className="law-card-header">
                                ...
                                {s.section && <span className="law-section-num">{s.section}</span>}
                            </div>
                            <h3 className="law-title">{s.title}</h3>
                            <p className="law-desc">{s.description}</p>
                            {s.applicability && <div className="law-meta"><strong>Applicability:</strong> {s.applicability}</div>}
                            {s.remedy && <div className="law-meta"><strong>Remedy:</strong> {s.remedy}</div>}
                            {s.authority && <div className="law-meta"><strong>Authority:</strong> {s.authority}</div>}
                        </motion.div>
                    ))}
                </div>
            )}
